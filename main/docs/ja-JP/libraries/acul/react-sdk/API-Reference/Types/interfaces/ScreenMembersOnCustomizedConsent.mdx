...
各オブジェクトには、アプリケーションが実行しようとしている特定のデータやアクションに関する詳細な情報が含まれます。
Rich Authorization Requests (RAR) や、その他のきめ細かい権限モデルに関連している場合があります。
このリストは、ユーザーが内容を確認できるように表示する必要があります。
...
通常、`read:profile` や `openid` のような比較的高レベルな権限が含まれます。
このリストは、ユーザーが内容を確認できるように表示する必要があります。
