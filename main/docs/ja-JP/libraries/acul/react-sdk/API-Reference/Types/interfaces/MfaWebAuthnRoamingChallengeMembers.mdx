...
export interface MfaWebAuthnRoamingChallengeMembers extends BaseMembers {
   ...
   *
   * @param {TryAnotherMethodOptions} [options] - リクエストのオプションのカスタムパラメータ。
...
`verify()` では自動的に処理されない形で失敗した場合や、開発者が別のアクションを試す前に
明示的にエラーを報告したい場合に使用します。
このメソッドは、`action: "showError::{errorDetails}"` とともにエラーの詳細を送信します。

エラー報告の送信が完了したときに解決される Promise を返します。
...
```typescript Example
// 'sdk' が MfaWebAuthnRoamingChallenge のインスタンスであると仮定
// また、webAuthnError が失敗した navigator.credentials.get() 呼び出しからのエラーオブジェクトであると仮定します。
try {
  await sdk.reportWebAuthnError({
    error: { name: webAuthnError.name, message: webAuthnError.message }
  });
...
WebAuthn API からの `name` と `message` を含む `error` オブジェクトと、
その他のカスタムオプションを含みます。
...
アクションの送信が完了したときに解決される Promise を返します。
...
try {
  await sdk.tryAnotherMethod();
  // 成功時、Auth0 が MFA 選択画面へのリダイレクトを処理します。
} catch (error) {
  console.error("Failed to switch MFA method:", error);
}
```
...
成功した場合、結果として得られた credential を `action: "default"` とともに Auth0 に送信します。
...
検証の試行が送信されたときに解決される Promise を返します。
成功した処理は通常リダイレクトにつながるため、データを直接返しません。
...
`rememberDevice` などの検証処理用の任意パラメーターや、
その他のカスタムオプションです。

**プロパティ**

<ParamField body='rememberDevice?' type='boolean'>
任意。true の場合、今後の MFA チャレンジのためにブラウザーを記憶しようとします。
