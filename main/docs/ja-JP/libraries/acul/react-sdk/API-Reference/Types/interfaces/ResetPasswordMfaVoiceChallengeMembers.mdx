...
`reset-password-mfa-voice-challenge` 画面で使用可能なメソッドとプロパティを定義するインターフェース。
...
チャレンジが送信されると解決される Promise を返します。
...
コードが再送信されると解決される Promise を返します。
...
アクションが完了すると解決される Promise を返します。
...
リクエストが送信されると解決される Promise を返します。
