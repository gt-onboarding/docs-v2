...
WebAuthn チャレンジのオプションや、デバイスを記憶するかどうかの設定を含む、画面固有のプロパティとデータ。
...
このメソッドは、`action: "showError::{errorDetailsJsonString}"` と空の `response` を含むエラー詳細を送信します。

エラー報告が送信されると解決される Promise を返します。
...
// また、webAuthnDomException は失敗した navigator.credentials.get() 呼び出しからのエラーオブジェクトです。
try {
  await sdk.reportBrowserError({
    error: { name: webAuthnDomException.name, message: webAuthnDomException.message }
  });
...
WebAuthn API の DOMException からの `name` および `message` を含む `error` オブジェクトと、その他の任意のカスタムオプションを含みます。
...
ユーザーが WebAuthn プラットフォームチャレンジをスキップして、別の MFA（多要素認証）手段を選択できるようにします。
このアクションは `action: "pick-authenticator"` を Auth0 に送信し、
ユーザーを MFA 要素選択画面へ遷移させます。

`pick-authenticator` アクションが送信されると解決される Promise を返します。
...
このメソッドは、`screen.publicKey` で指定されたチャレンジオプションを使用して
内部的に `navigator.credentials.get()` を呼び出します。
成功した場合、`action: "default"` を指定して結果の認証情報を Auth0 に送信します。

`navigator.credentials.get()` が失敗した場合（例: ユーザーによるキャンセル、タイムアウト、その他の WebAuthn API エラー）には、
エラーの詳細を指定して `reportBrowserError` を呼び出すことを推奨します。

検証試行が送信されたときに解決される Promise。
                         通常、成功するとリダイレクトが発生します。

#### 例外

`screen.publicKey` が存在しない場合、
                `navigator.credentials.get()` が `getPasskeyCredentials` で処理されない予期しないエラーで失敗した場合、
                あるいは Auth0 へのフォーム送信が失敗した場合にエラーをスローします。
...
try {
  ...
  // WebAuthn API エラーの場合は、それを報告する
  if (error.name && error.message) { // DOMException のような error かどうかの基本的なチェック
    await sdk.reportBrowserError({ error: { name: error.name, message: error.message } });
  }
...
`screen.showRememberDevice` が true の場合の `rememberDevice` や、その他のカスタムオプションなどを指定できます。

**プロパティ**

<ParamField body='rememberDevice?' type='boolean'>
省略可能。true の場合、今後の MFA（多要素認証）チャレンジ時のためにブラウザーを記憶しようとします。
