...
MFA（多要素認証）の WebAuthn プラットフォーム登録に固有のデータ構造を持つ画面オブジェクト。
...
現在のデバイスでの WebAuthn プラットフォーム登録を拒否します。
このアクションは、ユーザーがこの特定のデバイスでプラットフォーム認証器を使用したくないことを示します。

拒否アクションが正常に送信されると解決される Promise を返します。
...
エラー報告が正常に送信されると解決される Promise を返します。
...
```typescript Example
// 'sdk' が MfaWebAuthnPlatformEnrollment のインスタンスであると仮定
// パスキー作成試行（例: submitPasskeyCredential から）の catch ブロック内:
} catch (webAuthnError) {
  if (webAuthnError.name) { // WebAuthn API エラーの可能性があるかを確認
     await sdk.reportBrowserError({ error: { name: webAuthnError.name, message: webAuthnError.message } });
  }
  // UI を更新してユーザーにエラーメッセージを表示
}
```
...
WebAuthn プラットフォーム登録をスヌーズ（延期）します。
このアクションは通常、ユーザーが後で登録するよう再度リマインドされることを意味します。

スヌーズアクションが正常に送信されると解決される Promise を返します。
...
// 'sdk' が MfaWebAuthnPlatformEnrollment のインスタンスであると仮定
try {
  if (!sdk.screen.publicKey) { // オプションが利用可能かを確認
  ...
  // WebAuthn API エラーである場合は、reportBrowserError を呼び出すなどしてエラーを処理
  if (error.name && error.message) { // WebAuthn エラーらしいかどうかを確認
    await sdk.reportBrowserError({ error: { name: error.name, message: error.message } });
  }
}
```
...
<Expandable title="パラメーター">
  <ParamField body='payload?' type={<span><a href="/docs/libraries/acul/js-sdk/Screens/interfaces/SubmitPasskeyCredentialOptions">SubmitPasskeyCredentialOptions</a></span>}>
    作成された認証情報と共にサーバーに送信される任意のカスタムパラメーター。
  </ParamField>
</Expandable>
