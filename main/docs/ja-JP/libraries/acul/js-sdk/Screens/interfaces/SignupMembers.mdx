...
export interface SignupMembers extends BaseMembers {
  ...
  /**
 * 現在のトランザクション状態に基づいて、有効な識別子(例: メール、電話番号、ユーザー名)のリストを返します。
 *
 * 識別子は、接続戦略および認証またはサインアップフロー中に提供される設定に応じて、必須またはオプションになります。
 *
...
<ParamField body="getSignupIdentifiers" type={<span><a href="/docs/libraries/acul/js-sdk/Screens/interfaces/Identifier">Identifier</a>[]</span>}>
  **`ユーティリティ機能`**
  ...
  現在のトランザクションの state に基づいて、有効になっている識別子（例: email、phone、username）の一覧を返します。

  識別子が必須か任意かは、接続戦略と、認証またはサインアップフロー中に指定される設定に応じて変わります。

  有効な [Identifier](/docs/ja-JP/libraries/acul/js-sdk/Screens/interfaces/Identifier) オブジェクトの配列、またはトランザクションが初期化されていない場合は `null` を返します。
