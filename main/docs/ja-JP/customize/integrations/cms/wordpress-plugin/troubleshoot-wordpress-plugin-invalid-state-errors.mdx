...
Pantheonなど、ホストによっては、特定のCookie名の使用を要求するところもあります。テーマまたはカスタムプラグインで`auth0_state_cookie_name`フィルターを使用して、Cookie名を変更できます。`auth0_state_cookie_name`フィルターの詳細については、[Auth0 Wordpressプラグインのログイン時間を延長する](/docs/ja-JP/ja-jp/customize/integrations/cms/wordpress-plugin/extend-login-by-auth0)をお読みください。その他の情報については、[関連するGitHubの問題を参照し](https://github.com/auth0/wp-auth0/issues/494)、[その修正方法を確認して](https://github.com/auth0/wp-auth0/pull/495)ください。
...
* Auth0プラグインのバージョン
* テストに使用したブラウザーとOS
* ログインフォームを使ってページを読み込むところから、「無効な状態」メッセージを受け取るところまで、処理の全体を記録したHARファイルHARファイルの詳細については、[HARファイルの生成と分析](/docs/ja-JP/ja-jp/troubleshoot/troubleshooting-tools/generate-and-analyze-har-files)をお読みください。
