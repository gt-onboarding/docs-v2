...
Auth0をネイティブアプリケーションと統合するには、まず、<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-JP/ja-jp/glossary?term=auth0-dashboard" tip="Auth0 Dashboard: サービスを構成するためのAuth0の主製品。" cta="用語集の表示">Auth0 Dashboard</Tooltip>を使ってアプリをAuth0に登録する必要があります。デバイス（i0S、Androidなど）でネイティブ実行されているモバイル、デスクトップ、ハイブリッドのアプリが含まれます。
...
3. モバイルアプリを開発している場合には、iOSまたはAndroidに必要なパラメーターを **［Advanced Settings（詳細設定）］** に入力してから、 **［Save Changes（変更の保存）］** をクリックします。
   ...
   * iOSアプリには、[**［Team ID（チームID）］** ](/docs/ja-JP/ja-jp/get-started/applications/enable-universal-links-support-in-apple-xcode)[と](/docs/ja-JP/ja-jp/get-started/applications/enable-universal-links-support-in-apple-xcode)[**［App Bundle Identifier（アプリのバンドル識別子）］** ](/docs/ja-JP/ja-jp/get-started/applications/enable-universal-links-support-in-apple-xcode)を入力します。
   * Androidアプリには、[**［App Package Name（アプリのパッケージ名）］** ](/docs/ja-JP/ja-jp/get-started/applications/enable-android-app-links-support)[と](/docs/ja-JP/ja-jp/get-started/applications/enable-android-app-links-support)[**［Key Hashes（キーハッシュ）］** ](/docs/ja-JP/ja-jp/get-started/applications/enable-android-app-links-support)を入力します。
