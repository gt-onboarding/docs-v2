...
<Tooltip data-tooltip-id="react-containers-DefinitionTooltip-0" href="/docs/ja-JP/ja-jp/glossary?term=openid" tip="OpenID: アプリケーションがログイン情報を収集および保存することなくにユーザーのIDを検証できるようにする認証用のオープン標準。" cta="用語集の表示">OpenID</Tooltip> FAPI適合性テストに合格するには、まず次の設定を行います。

* `compliance_level`プロパティを、`fapi1_adv_pkj_par`または`fapi1_adv_mtls_par`のいずれかの目的のプロファイルに設定します
* [mTLS（](/docs/ja-JP/ja-jp/get-started/applications/configure-mtls)[mTLS](/docs/ja-JP/ja-jp/get-started/applications/configure-mtls/configure-mtls-for-a-tenant#enable-mtls-aliases)[エイリアスを含む）を構成](/docs/ja-JP/ja-jp/get-started/applications/configure-mtls)するか、[秘密鍵JWTを構成](/docs/ja-JP/ja-jp/get-started/applications/configure-private-key-jwt)
* [mTLSトークンバインディングを構成します](/docs/ja-JP/ja-jp/get-started/applications/configure-mtls/configure-mtls-for-a-client#enable-token-binding)
...
デフォルトでは、Auth0はスコープのない要求を許可します。スコープが存在しない場合は、`openid`スコープであると想定します。Auth0は、[アクション](/docs/ja-JP/ja-jp/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions)で設定できる`redirect_uri`のない要求も許可します。ただし、FAPI適合テストでは、Auth0の制限を強化する必要があります。
...
* [mTLS認証の設定](/docs/ja-JP/ja-jp/get-started/applications/configure-mtls)