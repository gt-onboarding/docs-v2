...
4. **［Disable Sign Ups（サインアップを無効にする）］** かどうかを決定します。この設定を有効にすると、既存のユーザーに対してのみパスワードレスのアクセスを許可できますが、アプリケーションがユーザー列挙攻撃の脅威にさらされる可能性があります。詳しくは、[パスワードレス接続のベストプラクティス](/docs/ja-JP/ja-jp/authenticate/passwordless/best-practices)をお読みください。
...
ユーザーが[組織](/docs/ja-JP/ja-jp/manage-users/organizations/organizations-overview)を通じてログインしている場合は、次のパラメータも利用できます：
...
[独自のSMTPメールプロバイダーを構成](/docs/ja-JP/ja-jp/customize/email/smtp-email-providers)して、メール通信を管理、監視、トラブルシューティングし、メールテンプレートをカスタマイズします。
...
* [カスタマイズされたSMTP外部メールプロバイダー](/docs/ja-JP/ja-jp/customize/email/smtp-email-providers/configure-custom-external-smtp-email-provider)