...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Clés d’identification natives est actuellement proposé en accès anticipé limité. Pour en savoir plus sur les étapes de diffusion d’Auth0, consultez [Étapes de la diffusion du produit](/docs/fr-CA/fr-ca/troubleshoot/product-lifecycle/product-release-stages).
</Callout>
...
* [Clés d’identification de connexion universelle](/docs/fr-CA/fr-ca/authenticate/database-connections/passkeys) pour les applications Web.
...
Les clés d’identification natives utilisent une combinaison du [Auth0 Authentication API](/docs/fr-CA/fr-ca/native-passkeys-api) et des API iOS ou Android natives pour intégrer les flux de défis-réponse directement dans votre application mobile. Cela vous permet de créer une expérience d’inscription et de connexion intégrée pour votre application qui ne repose pas sur la redirection des utilisateurs par leur navigateur pour compléter l’authentification.
...
Appelez le point de terminaison [Update a Client (Mettre à jour un client)](/docs/fr-CA/fr-ca/api/management/v2/clients/patch-clients-by-id) et :
...
2. Votre application lance ensuite le défi-réponse d’inscription en appelant le point de terminaison [Demander un défi-réponse d’inscription](/docs/fr-CA/fr-ca/native-passkeys-api#request-signup-challenge) d’Auth0 Authentication API :

   <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
     * Si vous ne spécifiez pas de `realm`, le répertoire par défaut de votre locataire est utilisé.
     * Par défaut, le courriel est l’identifiant requis. Si vous avez activé [Identifiants flexibles](/docs/fr-CA/fr-ca/authenticate/database-connections/activate-and-configure-attributes-for-flexible-identifiers) pour votre connexion de base de données, vous pouvez utiliser une combinaison de `email`, `phone_number`, ou `username` à la place. Ces options peuvent être obligatoires ou facultatives et doivent correspondre à votre configuration d’identificateur flexible.
   </Callout>
...
5. Votre application utilise ensuite les identifiants obtenus lors du processus d’enregistrement, y compris les détails `authn_response`, pour appeler le point de terminaison [Jeton](/docs/fr-CA/fr-ca/native-passkeys-api#authenticate-new-user) :
...
1. Un utilisateur lance votre application et commence le processus de connexion. Votre application lance ensuite le défi-réponse de connexion en appelant le point de terminaison[Demander un défi-réponse de connexion](/docs/fr-CA/fr-ca/native-passkeys-api#request-login-challenge) d’Authentication API :
...
4. Votre application utilise ensuite les identifiants obtenus lors du processus de connexion, y compris les détails `authn_response`, pour appeler le point de terminaison[Jeton](/docs/fr-CA/fr-ca/native-passkeys-api#authenticate-existing-user) :
...
* Auth0 Authentication API (API d’authentification Auth0)
  ...
  * [Demander un défi-réponse d’inscription](/docs/fr-CA/fr-ca/native-passkeys-api#request-signup-challenge)
  * [Authentifier un nouvel utilisateur](/docs/fr-CA/fr-ca/native-passkeys-api#authenticate-new-user)
  * [Demander un défi-réponse de connexion](/docs/fr-CA/fr-ca/native-passkeys-api#request-login-challenge)
  * [Authentifier un utilisateur existant](/docs/fr-CA/fr-ca/native-passkeys-api#authenticate-existing-user)
* Ressources Android
