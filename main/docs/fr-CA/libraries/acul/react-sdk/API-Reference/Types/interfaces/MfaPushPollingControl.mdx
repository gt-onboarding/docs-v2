...
Interface de contrôle pour gérer une session d’interrogation pour les notifications push d’AMF (MFA).

Cette interface fournit des méthodes impératives pour démarrer, arrêter et inspecter l’état
d’une boucle d’interrogation de longue durée qui vérifie si un défi push d’AMF a été
approuvé.
...
Indique si le processus de sondage est actuellement en cours d'exécution.
...
- Si le sondage est déjà actif, un nouvel appel n'a aucun effet.
- S'il a été arrêté précédemment, un nouvel appel relance la boucle de sondage.

```ts Example
control.startPolling(); // Commence à vérifier le défi AMF (MFA) push
```
...
- Une fois arrêté, `isRunning()` retourne `false`.
- Peut être appelé en toute sécurité plusieurs fois ; les appels suivants n'ont aucun effet.
