...
export interface EmailVerificationResultMembers extends BaseMembers {
  /**
   * Fournit l'accès aux propriétés et données spécifiques de l'écran Résultat de vérification de courriel,
   * incluant le `status` de vérification et le lien de `connexion`.
