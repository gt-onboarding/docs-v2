...
export interface MfaWebAuthnRoamingChallengeMembers extends BaseMembers {
   ...
   * @throws {Error} Lance une erreur si `screen.data.publicKeyChallengeOptions` est manquant,
...
Propriétés et data spécifiques à l'écran, y compris les options de challenge WebAuthn.
...
```typescript Example
// En supposant que 'sdk' soit une instance de MfaWebAuthnRoamingChallenge
// Et que webAuthnError soit un objet d’erreur provenant d’un appel navigator.credentials.get() ayant échoué.
try {
  await sdk.reportWebAuthnError({
    error: { name: webAuthnError.name, message: webAuthnError.message }
  });
} catch (submitError) {
  console.error("Échec de l’envoi du rapport d’erreur WebAuthn :", submitError);
}
```
...
Permet à l’utilisateur de se soustraire au défi WebAuthn et de sélectionner une autre méthode d’AMF (MFA).
...
Paramètres personnalisés optionnels pour la requête.
</ParamField>
...
options fournies dans `screen.data.publicKeyChallengeOptions`.
En cas de réussite, il soumet les informations d’identification résultantes à Auth0 avec `action: "default"`.
Si `navigator.credentials.get()` échoue avec une erreur connue de l’API WebAuthn (comme `NotAllowedError`),
cette méthode appellera `reportWebAuthnError` en interne pour informer Auth0.

Une promesse qui se résout lorsque la tentative de vérification est soumise.
                         Elle ne renvoie pas directement de data, car une opération réussie entraîne généralement une redirection.

<div id="throws">
  #### Exceptions levées
</div>

Lève une erreur si `screen.data.publicKeyChallengeOptions` est manquant,
                si `navigator.credentials.get()` échoue avec une erreur inattendue,
...
Optionnel. Si true, tente de mémoriser le navigateur pour les futurs défis d’AMF.
