---
title: "MembresMfaWebAuthnRoamingEnrollment"
---

Interface définissant les méthodes et propriétés disponibles pour l&#39;écran `mfa-webauthn-roaming-enrollment` d&#39;inscription à l’AMF (MFA) WebAuthn itinérante.
...
export interface MfaWebAuthnRoamingEnrollmentMembers extends BaseMembers {
  ...
  /**
   * Lance la création des justificatifs d'identité WebAuthn et soumet le résultat au serveur.
   * Cela correspond à l'interaction de l'utilisateur avec l'invite de clés de sécurité FIDO.
   ...
   *   // Vous sérialiseriez normalement la réponse des justificatifs d'identité ici
   ...
   *   // Gérez l'erreur, par exemple, affichez un message d'erreur à l'utilisateur ou soumettez les détails de l'erreur
  ...
  /**
   * Soumet les détails d'une erreur WebAuthn du navigateur au serveur.
   * Cela est utilisé lorsque l'API WebAuthn du navigateur rencontre une erreur.
   ...
   *
   * const webauthnEnrollment = new MfaWebAuthnRoamingEnrollment();
   * // Exemple de gestionnaire d'erreur pour l'appel de l'API WebAuthn du navigateur
...
Démarre la création de l’information d’identification WebAuthn et envoie le résultat au serveur.
...
try {
  ...
  const credential = await navigator.credentials.create({ publicKey: webauthnEnrollment.screen.data.passkey.public_key });
  // Vous sérialiseriez généralement ici la réponse d’authentification
  const response = JSON.stringify(credential);
  await webauthnEnrollment.enroll({ response });
} catch (error) {
  console.error('WebAuthn enrollment failed:', error);
...
Envoie au serveur les détails d’une erreur WebAuthn survenue dans le navigateur.
