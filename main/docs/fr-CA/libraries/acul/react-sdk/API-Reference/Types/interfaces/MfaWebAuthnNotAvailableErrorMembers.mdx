---
title: "MfaWebAuthnNotAvailableErrorMembers"
---

MfaWebAuthnNotAvailableErrorMembers

````ts Example
export interface MfaWebAuthnNotAvailableErrorMembers extends BaseMembers {
  /**
   * Permet à l'utilisateur de sélectionner une méthode AMF (MFA) différente lorsque WebAuthn n'est pas disponible.
   * Cette action dirige l'utilisateur vers un écran de sélection d'authentificateur où il peut choisir
   * parmi d'autres facteurs AMF (MFA) disponibles ou inscrits.
   *
   * @param {CustomOptions} [payload] - Paramètres personnalisés facultatifs à envoyer avec la requête.
   * @returns {Promise<void>} Une promesse qui se résout lors de la soumission réussie de l'action « pick-authenticator ».
   * @throws {Error} Lance une erreur si la soumission du formulaire échoue (p. ex., problèmes réseau, erreurs de validation côté serveur).
   * @example
   * ```typescript
   * // En supposant que « sdk » est une instance de MfaWebAuthnNotAvailableError
   * try {
   *   await sdk.tryAnotherMethod();
   *   // En cas de succès, Auth0 gère la redirection vers l'écran de sélection d'authentificateur.
   * } catch (error) {
   *   console.error('Échec du basculement vers une autre méthode AMF (MFA) :', error);
   *   // Mettre à jour l'interface utilisateur au besoin pour informer l'utilisateur de l'échec.
   * }
   * ```
   */
  tryAnotherMethod(payload?: CustomOptions): Promise<void>;
}
````


<div id="properties">
  ## Propriétés
</div>

<ParamField body='branding' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/BrandingMembers">BrandingMembers</a></span>}/>

<ParamField body='client' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ClientMembers">ClientMembers</a></span>}/>

<ParamField body='organization' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/OrganizationMembers">OrganizationMembers</a></span>}/>

<ParamField body='prompt' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/PromptMembers">PromptMembers</a></span>}/>

<ParamField body='screen' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/ScreenMembers">ScreenMembers</a></span>}/>

<ParamField body='tenant' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/TenantMembers">TenantMembers</a></span>}/>

<ParamField body='transaction' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/TransactionMembers">TransactionMembers</a></span>}/>

<ParamField body='untrustedData' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/UntrustedDataMembers">UntrustedDataMembers</a></span>}/>

<ParamField body='user' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/UserMembers">UserMembers</a></span>}/>

<div id="methods">
  ## Méthodes
</div>

<ParamField body='tryAnotherMethod' type='Promise<void>'>

Permet à l’utilisateur de choisir une autre méthode d’AMF (MFA), car WebAuthn n’est pas disponible.
Cette action amène l’utilisateur à un écran de sélection d’authentificateur où il peut choisir
parmi d’autres facteurs d’AMF disponibles/inscrits.

Promesse qui se résout après l’envoi réussi de l’action 'pick-authenticator'.

<div id="throws">
  #### Lève
</div>

Lève une erreur si l’envoi du formulaire échoue (p. ex., problèmes réseau, erreurs de validation côté serveur).

```typescript Example
// En supposant que 'sdk' est une instance de MfaWebAuthnNotAvailableError
try {
  await sdk.tryAnotherMethod();
  // En cas de réussite, Auth0 gère la redirection vers l’écran de sélection d’authentificateur.
} catch (error) {
  console.error('Échec du basculement vers une autre méthode d’AMF (MFA) :', error);
  // Éventuellement mettre à jour l’interface utilisateur afin d’informer l’utilisateur de l’échec.
}
```

<Expandable title="Paramètres">
<ParamField body='payload?' type={<span><a href="/docs/libraries/acul/react-sdk/API-Reference/Types/interfaces/CustomOptions">CustomOptions</a></span>}>


Paramètres personnalisés facultatifs à envoyer avec la requête.

</ParamField>

</Expandable>
</ParamField>