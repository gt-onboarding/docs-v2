...
Propriétés et données propres à l'écran, y compris les options de challenge WebAuthn et la préférence de mémorisation de l'appareil.
...
d’un problème WebAuthn côté navigateur (p. ex., annulation par l’utilisateur `NotAllowedError`, délai dépassé).
...
provenant de l’exception DOM de l’API WebAuthn) et tout autre paramètre personnalisé.
...
Permet à l’utilisateur d’abandonner le défi de plateforme WebAuthn et de sélectionner une autre méthode d’AMF (MFA).
...
try {
  await sdk.tryAnotherMethod();
  // En cas de succès, Auth0 gère la redirection vers la sélection de la méthode d’AMF.
} catch (error) {
  console.error("Failed to switch MFA method:", error);
}
```
...
Déclenche le défi de l’authentificateur de plateforme WebAuthn.
Cette méthode appelle en interne `navigator.credentials.get()` en utilisant les options de défi
fournies dans `screen.publicKey`.
En cas de succès, elle envoie les informations d’identification obtenues à Auth0 avec `action: "default"`.
...
Promesse qui se résout lorsque la tentative de vérification est soumise.
                         Une opération réussie entraîne généralement une redirection.
...
Optionnel. Si true, tente de se souvenir du navigateur pour de futurs défis d’AMF (MFA).
