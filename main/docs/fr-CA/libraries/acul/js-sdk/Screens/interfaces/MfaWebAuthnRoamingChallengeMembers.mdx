...
Permet à l’utilisateur de se retirer du défi WebAuthn et de sélectionner une autre méthode d’AMF (MFA).
...
Lance le défi WebAuthn en appelant `navigator.credentials.get()` avec les
options fournies dans `screen.data.publicKeyChallengeOptions`.
En cas de succès, la méthode soumet les informations d’identification obtenues à Auth0 avec `action: "default"`.
...
Une promesse qui se résout lorsque la tentative de vérification est soumise.
                         Elle ne retourne pas directement de données, car une opération réussie se traduit généralement par une redirection.
...
Facultatif. Si true, tente de mémoriser le navigateur pour de futurs défis d’AMF (MFA).
