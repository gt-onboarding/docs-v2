...
continue
Soumet le choix de la méthode d’envoi d’AMF (MFA) (SMS ou appel vocal) pour poursuivre le défi.
Cette action déclenchera l’envoi du code de vérification au téléphone de l’utilisateur.
...
Cette action redirige généralement l’utilisateur vers un écran où il peut sélectionner un numéro de téléphone parmi plusieurs numéros inscrits, le cas échéant.
...
Lance l’action permettant de sélectionner un autre facteur/authentificateur d’AMF (MFA).
Cette action redirige l’utilisateur vers un écran où il peut choisir une autre méthode d’AMF (p. ex., application OTP, code de récupération).
