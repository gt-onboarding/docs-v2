...
export interface ScreenMembersOnMfaWebAuthnEnrollmentSuccess extends ScreenMembers {
  ...
  data: {
     ...
     * - `'webauthn-roaming'` : Indique un authentificateur nomade tel qu'une clé de sécurité USB.
     * - `'webauthn-platform'` : Indique un authentificateur de plateforme tel que Touch ID ou Windows Hello.
     * @type {WebAuthnType}
     */
...
Le surnom que l’utilisateur (ou le système) a attribué au nouvel authentificateur WebAuthn inscrit.
Cela aide l’utilisateur à identifier l’authentificateur plus tard.
