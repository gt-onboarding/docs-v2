...
L’objet d’écran contenant la structure de données spécifique à l’erreur AMF (MFA) WebAuthn.
...
```typescript Example
// En supposant que « sdk » soit une instance de MfaWebAuthnError
await sdk.noThanks();
```
...
Permet à l'utilisateur de choisir une autre méthode d'AMF (MFA) si d'autres facteurs sont inscrits ou disponibles.
Cela redirige l'utilisateur vers un écran de sélection d'authentificateur.
...
Permet à l'utilisateur d'opter pour une authentification basée sur un mot de passe comme méthode alternative, si elle est configurée.
