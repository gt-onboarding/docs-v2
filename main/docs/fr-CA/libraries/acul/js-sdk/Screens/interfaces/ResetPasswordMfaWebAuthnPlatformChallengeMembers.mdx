...
En cas de réussite, l’assertion d’authentification résultante est envoyée à Auth0 avec `action: "default"`.
...
Génère une erreur si `screen.publicKey` est manquant,
...
try {
  ...
  // Vérifiez sdk.transaction.errors pour des messages de validation côté serveur si la page est rechargée.
}
```
...
Optionnel. Si `true` et `screen.showRememberDevice` est également `true`,
cela indique que l’utilisateur a choisi de mémoriser le navigateur actuel,
ce qui peut permettre de sauter l’AMF pendant une certaine période. Correspond au champ de formulaire `rememberBrowser`.
</ParamField>
</ParamField>
...
Ceci est utilisé pour informer Auth0 de problèmes comme l’annulation par l’utilisateur, un délai d’attente ou du matériel non pris en charge.
Les détails de l’erreur sont envoyés avec `action: "showError::{errorDetailsJsonString}"`.

Une promesse qui est résolue lorsque le rapport d’erreur est envoyé.
...
try {
  ...
  // Auth0 peut réafficher la page ou effectuer une redirection selon l’erreur.
} catch (submitError) {
  console.error("Échec de l’envoi du rapport d’erreur WebAuthn du navigateur :", submitError);
}
```
...
provenant de la DOMException de l’API WebAuthn) et toute option personnalisée supplémentaire.
...
Permet à l'utilisateur de se désengager du challenge de plateforme WebAuthn et de sélectionner une autre méthode d'AMF (MFA).
Cette action envoie `action: "pick-authenticator"` à Auth0, ce qui devrait rediriger
l'utilisateur vers un écran de sélection du facteur d'AMF pour le flux de réinitialisation de mot de passe.
...
try {
  await sdk.tryAnotherMethod();
  // En cas de réussite, Auth0 gère la redirection vers la sélection du facteur d'AMF.
} catch (error) {
  console.error("Failed to switch MFA method:", error);
}
```
