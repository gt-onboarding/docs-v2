...
La documentation ci-dessous part du principe que vous avez suivi les étapes des [sections sur l’installation et le démarrage](/docs/fr-CA/fr-ca/libraries/auth0-php), et que vous avez utilisé le code fourni.

<div id="authorization-code-flow">
  ## Flux de code d’autorisation
</div>

Un [Flux de code d’autorisation](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow) est la façon de base de donner accès aux utilisateurs à votre application. Ce flux est le même utilisé sur la [Utilisation de base d’Auth0-PHP](/docs/fr-CA/fr-ca/libraries/auth0-php/auth0-php-basic-use). Si vous avez besoin d’un contrôle plus granulaire sur le processus de connexion ou de Callback, cette section explique comment utiliser directement l’Authentication API.
...
2. Nous vérifions que nous avons une valeur `state` et qu’elle coïncide à la même que celle générée. [Ceci est important pour éviter les attaques CSRF.](/docs/fr-CA/fr-ca/secure/attack-protection)
...
Consultez [Utilisation de Management API avec Auth0-PHP](/docs/fr-CA/fr-ca/libraries/auth0-php/using-the-management-api-with-auth0-php) pour plus d’informations sur la façon d’utiliser ce jeton d’accès.
