...
Configurez votre connexion à la base de données pour migrer automatiquement vos utilisateurs d’un magasin d’utilisateurs externe vers Auth0 à l’aide de [scripts d’action de base de données personnalisés](/docs/fr-CA/fr-ca/authenticate/database-connections/custom-db/templates).
...
Le script [Connexion](/docs/fr-CA/fr-ca/authenticate/database-connections/custom-db/templates/login) s’exécute chaque fois qu’un utilisateur non trouvé dans Auth0 tente de se connecter. Il vérifie que l’utilisateur existe dans le magasin d’utilisateurs externe sans redemander le mot de passe à ce dernier.

<div id="get-user">
  ### Obtenir un utilisateur
</div>

Le script [Obtenir l’utilisateur](/docs/fr-CA/fr-ca/authenticate/database-connections/custom-db/templates/get-user) s’exécute après l’un des scénarios suivants :
...
* Il est fait appel au point de terminaison de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
  Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip> [mettre à jour le courriel ou le nom d’utilisateur](/docs/fr-CA/fr-ca/api/v2#!/Users/patch_users_by_id) ou [créer un utilisateur](/docs/fr-CA/fr-ca/api/management/v2#!/Users/post_users) .
...
2. Utilisez le point de terminaison [List or Search Users (Lister ou Rechercher des utilisateurs)](/docs/fr-CA/fr-ca/api/v2#!/Users/get_users) de Management API.
...
Si vous rencontrez des problèmes avec la migration automatique, lisez premièrement [Les meilleures pratiques pour la connexion à la base de données personnalisée et les scripts d’action.](/docs/fr-CA/fr-ca/authenticate/database-connections/custom-db/custom-database-connections-scripts).
...
1. Vérifiez les instructions `console.log()` avec l’[extension Journaux Webtask en temps réel](/docs/fr-CA/fr-ca/customize/extensions/real-time-webtask-logs).
...
3. Supprimez l’utilisateur avec le point de terminaison [Delete a Connection User (Supprimer un utilisateur de connexion)](/docs/fr-CA/fr-ca/api/management/v2#!/Connections/delete_users_by_email) de Management API.
...
Vous pouvez atténuer ce scénario en créant une [Action](/docs/fr-CA/fr-ca/customize/actions) qui vérifie si le profil de l’utilisateur manque de métadonnées, les récupère depuis le magasin d’utilisateurs externe, puis les stocke dans Auth0.
