...
Pour en savoir plus sur `app_metadata` et `user_metadata`, consultez [Comprendre le fonctionnement des métadonnées dans les profils utilisateurs](/docs/fr-CA/fr-ca/manage-users/user-accounts/metadata).
...
Le tableau `user.mfa_factors` contient les inscriptions à <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
