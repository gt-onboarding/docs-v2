...
Pour en savoir plus sur les minuteries d’inactivité et les modalités de délai d’expiration, lisez [URL de déconnexion spécifiques à l’application](/docs/fr-CA/fr-ca/manage-users/sessions/configure-session-lifetime-settings).
...
Les détails du flux de production changeraient dans le cas d’une session de longue durée où la session d’application serait très probablement plus courte que celle d’[authentification unique (SSO)](/docs/fr-CA/fr-ca/authenticate/single-sign-on).

Pour en savoir plus, lisez [Limites de durée de vie de la session](/docs/fr-CA/fr-ca/manage-users/sessions/configure-session-lifetime-settings) et [Obtenir des jetons d’actualisation](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/update-access-token-lifetime).
