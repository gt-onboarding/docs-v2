...
La durée de vie de la session détermine combien de temps le système doit conserver la session de connexion et est configurée par locataire. L’**expiration absolue** d’une session est définie lors de la création de ladite session. Vous pouvez activer des paramètres **non persistants** pour les sessions utilisateur, ou permettre à la session de continuer même si le navigateur est fermé. Vous pouvez ajuster l’Expiration absolue et les paramètres non-persistants dans Dashboard ou <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>. Pour en savoir plus, consultez [Configurer les paramètres de durée de vie des sessions](/docs/fr-CA/fr-ca/manage-users/sessions/configure-session-lifetime-settings).

<div id="cookies">
  ## Témoins
</div>

La plateforme Auth0 utilise trois types de témoins : de session, d’authentification multifacteur (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
Pour en savoir plus sur les témoins, veuillez consulter [Témoins de Authentication API](/docs/fr-CA/fr-ca/manage-users/cookies/authentication-api-cookies).
