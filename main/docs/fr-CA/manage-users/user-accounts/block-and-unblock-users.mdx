...
* bloqué par un administrateur via le <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
  Principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire">Auth0 Dashboard</Tooltip>
* bloqué via le <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
<div id="unblock-a-user-who-was-automatically-blocked">
...
Utilisez le [point de terminaison Débloquer par identifiant](/docs/fr-CA/fr-ca/api/management/v2#!/User_Blocks/delete_user_blocks) pour transmettre un identifiant (nom d’utilisateur, numéro de téléphone ou courriel) :

```json lines
curl -X DELETE  https://{yourAuth0Tenant}.auth0.com/api/v2/user-blocks/john.doe%40gmail.com
```

Ou utilisez le point de terminaison [Mettr](/docs/fr-CA/fr-ca/api/management/v2#%5C!/Users/patch_users_by_id)[e à jour un utilisateur](/docs/fr-CA/fr-ca/api/management/v2#%5C!/Users/patch_users_by_id) pour corriger l’identifiant `user_id`:

```json lines
curl -X PATCH -H "Content-Type: application/json" -d '{"blocked":false}' https://{yourAuth0Tenant}.auth0.com/api/v2/users/{user_id}
```

<div id="unblock-a-user-who-was-blocked-through-the-user-profile">
...
Pour débloquer un utilisateur à l’aide de Management API, utilisez le point de terminaison [Mettre à jour un utilisateur](/docs/fr-CA/fr-ca/api/management/v2#!/Users/patch_users_by_id) pour définit l’attribut `blocked` à `false` :
...
* [Gérer l’accès des utilisateurs aux applications](/docs/fr-CA/fr-ca/manage-users/user-accounts/manage-user-access-to-applications)