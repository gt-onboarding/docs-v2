...
1. Connectez l’utilisateur à votre application.
   ...
   L’utilisateur s’authentifie auprès de votre application à l’aide de la [connexion universelle](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login). Pour en savoir plus, consultez [Démarrage rapide des applications Web ordinaires](/docs/fr-CA/fr-ca/quickstart/webapp), demandant un jeton pour le public de Management API Auth0 (audience=`https://{yourDomain}/api/v2/`).
...
2. Recherchez les utilisateurs dont l’adresse courriel est identique.
   ...
   Pour obtenir une liste de tous les enregistrements d’utilisateurs ayant la même adresse courriel, votre application appelle le point de terminaison [Get Users By Email endpoint](/docs/fr-CA/fr-ca/api/v2#!/users-by-email/get_users_by_email) en utilisant un [jeton d’accès Management API](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens) avec la permission `read:users`.
...
5. Votre application appelle le point de terminaison [Associer un point de terminaison de compte utilisateur](/docs/fr-CA/fr-ca/api/v2#!/Users/post_identities) de l’API à l’aide d’un [Jeton d’accès Management API Access Token](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens) avec la permission `update:users`.
