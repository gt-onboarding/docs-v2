...
Prenons l’exemple d’un site web conforme à l’OIDC (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=openid" tip="OpenID
...
La plupart des types d’applications (telles que les applications web, les applications à page unique et les applications natives) devraient utiliser le [Flux de code d’autorisation avec PKCE](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce) pour faciliter l’authentification de la connexion. Ce flux implique l’échange d’un code d’autorisation contre des jetons.
...
Dans ce scénario, trois sessions sont créées : la **session locale** (storezero.io), la **session du serveur d’autorisation** (storezero.auth0.com) et une **session du fournisseur d’identité (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
Service de stockage et de gestion des identités numériques." cta="Voir le glossaire">IdP</Tooltip>)** (facebook.com).

La session de l’IdP sur le serveur de Facebook authentifie l’utilisateur et fournit une expérience <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
Cependant, les sessions locales ne sont pas disponibles pour les applications sans système dorsal, telles que les applications à page unique. Au lieu de cela, ces applications utilisent une approche différente connue sous le nom d’[authentification silencieuse](/docs/fr-CA/fr-ca/authenticate/login/configure-silent-authentication) pour maintenir la connexion des utilisateurs.
...
* [Limites de durée de vie des sessions](/docs/fr-CA/fr-ca/manage-users/sessions/session-lifetime-limits)
* [Configuration des paramètres de durée de vie des sessions](/docs/fr-CA/fr-ca/manage-users/sessions/configure-session-lifetime-settings)
* [Témoins](/docs/fr-CA/fr-ca/manage-users/cookies)
