...
* Un jeton est nécessaire pour effectuer des requêtes auprès de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
  Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>. Pour en savoir plus, lisez [Jetons d’accès à Management API](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens).
* Pour effectuer des recherches d’utilisateurs, la permission `read:users` est requise.
* Pour obtenir les résultats de recherche les plus récents, utilisez un point de terminaison **immédiatement cohérent** pendant les processus d’authentification, comme [Obtenir des utilisateurs par identifiant](/docs/fr-CA/fr-ca/manage-users/user-search/retrieve-users-with-get-users-by-id-endpoint) et [Obtenir des utilisateurs par courriel](/docs/fr-CA/fr-ca/manage-users/user-search/retrieve-users-with-get-users-by-email-endpoint). Les recherches effectuées à l’aide de ces points de terminaison reflètent les résultats de toutes les opérations d’écriture réussies, y compris celles qui ont eu lieu peu de temps avant votre requête.
