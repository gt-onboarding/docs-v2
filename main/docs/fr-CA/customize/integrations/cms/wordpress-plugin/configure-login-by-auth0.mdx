...
Connexion par Auth0 peut être configuré à l’aide de l’assistant de configuration dans le plugiciel ou manuellement pour plus de contrôle sur le processus. Pour en savoir plus, veuillez consulter [Installer la connexion par Auth0](/docs/fr-CA/fr-ca/customize/integrations/cms/wordpress-plugin/install-login-by-auth0). Les instructions ci-dessous peuvent également être utilisées si l’assistant de configuration n’a pas été complété ou dans le cadre de la résolution des problèmes de connexion.
...
<Warning>
  L’URL de rappel **ne doit pas** être mise en cache, sinon une erreur « État non valide » risque de se produire lors de la connexion. Pour en savoir plus, consultez [Dépannage des erreurs d’état non valide des modules d’extension WordPress](/docs/fr-CA/fr-ca/customize/integrations/cms/wordpress-plugin/troubleshoot-wordpress-plugin-invalid-state-errors).
</Warning>
...
Pour que votre site WordPress puisse effectuer certaines actions au nom de votre locataire Auth0, vous devrez autoriser l’application créée ci-dessus à accéder à <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
Pour savoir comment activer et configurer cette méthode de connexion, veuillez consulter [Fournisseurs d’identité sociale](/docs/fr-CA/fr-ca/authenticate/identity-providers/social-identity-providers).

<div id="enterprise-connection-setup">
  ### Configuration de la connexion d’entreprise
</div>

Pour savoir comment activer et configurer cette méthode de connexion, veuillez consulter [Enterprise Identity Providers](/docs/fr-CA/fr-ca/authenticate/identity-providers/enterprise-identity-providers) (Fournisseurs d’identité de l’entreprise).
...
* **Domaine personnalisé** : Le domaine personnalisé pour votre locataire, si configuré. Pour en savoir plus, veuillez consulter [Domaines personnalisés](/docs/fr-CA/fr-ca/customize/custom-domains). Le nom de l’option est `custom_domain`.
...
* **Algorithme de signature <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
  Format standard de jeton d’ID (et souvent de jeton d’accès) utilisé pour représenter en toute sécurité des demandes entre deux parties." cta="Voir le glossaire">JWT</Tooltip>** L’algorithme utilisé pour signer les jetons à partir de l’onglet Paramètres avancés de l’application, <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=oath2" tip="OAuth 2.0
...
* **Page de connexion universelle** : Redirige la page `wp-login.php` vers la page de connexion universelle pour l’authentification unique <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=single-sign-on" tip="Authentification unique (SSO)
...
Les options ici n’affectent pas la page de connexion universelle (pour en savoir plus sur les options de personnalisation, veuillez consulter [Connexion universelle Auth0](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login).
...
* **Couleur primaire** : Pour en savoir plus sur ce paramètre, veuillez consulter [Options de configuration Lock](/docs/fr-CA/fr-ca/libraries/lock/lock-configuration). Le nom de l’option est `primary_color`.
...
* [Dépannage du plugiciel WordPress Login by Auth0](/docs/fr-CA/fr-ca/customize/integrations/cms/wordpress-plugin/troubleshoot-login-by-auth0)
* [Plugiciel WordPress Extend Login by Auth0](/docs/fr-CA/fr-ca/customize/integrations/cms/wordpress-plugin/extend-login-by-auth0)