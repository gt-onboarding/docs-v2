...
Event streams can either be provisioned using the <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
Principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire">Auth0 dashboard</Tooltip> or the [Auth0 Management API](/docs/fr-CA/fr-ca/api/management/v2). If using the <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip>, before you can set up an event stream, you need to create a machine-to-machine (M2M) application and authenticate with a Management API token. For more information, review [Management API Access Tokens](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens).
...
6. Review [Get Management API Access Tokens](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens/get-management-api-access-tokens-for-production) to retrieve and store your access token.
...
<Tabs>
  <Tab title="Auth0 CLI">
    ...
    Before setting up an event stream, you need to identify the [event types](/docs/fr-CA/fr-ca/customize/events/event-types) you want to monitor. Then, you will use your AWS account ID and region to set up your event stream, as demonstrated below.
...
Before setting up an event stream, you need to identify the [event types](/docs/fr-CA/fr-ca/customize/events/event-types) you want to monitor. You will then use your webhook handler to create an event stream, as demonstrated below.
...
After the stream is active, you can test the event stream. For more information, review [**Event Testing, Observability, and Failure Recovery**](/docs/fr-CA/fr-ca/customize/events/event-testing-observability-and-failure-recovery).

<div id="webhooks">
  ## En savoir plus
</div>

* [Event Types](/docs/fr-CA/fr-ca/customize/events/event-types)
* [Event Testing, Observability, and Failure Recovery](/docs/fr-CA/fr-ca/customize/events/event-testing-observability-and-failure-recovery)
* [Events Best Practices](/docs/fr-CA/fr-ca/customize/events/events-best-practices)