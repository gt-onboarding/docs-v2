...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Pour certaines erreurs, l’utilisateur peut être envoyé vers la page [Erreur générique](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login/error-pages) au lieu de l’URL de redirection.
</Callout>

Vous pouvez éviter l’inclusion des adresses courriel dans les liens de redirection en mettant à jour les modèles de courriel via <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
* Si l’utilisateur s’est connecté via une [Organization](/docs/fr-CA/fr-ca/manage-users/organizations/organizations-overview) :
...
Ce courriel sera généré lors de l’envoi d’une invitation d’inscription à <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
Si un utilisateur est invité à devenir membre d’une [Organization](/docs/fr-CA/fr-ca/manage-users/organizations/organizations-overview), il recevra un courriel **User Invitation (Invitation de l’utilisateur)** contenant un lien URL. Lorsque l’utilisateur clique sur le lien, il sera redirigé vers votre route de connexion par défaut. Pour en savoir plus, consultez [Configurer les routes de connexion par défaut](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login/configure-default-login-routes).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ...
  Si l’utilisateur n’accepte pas une invitation avant qu’elle n’expire, l’invitation sera marquée comme expirée et éventuellement supprimée d’[Auth0 Dashboard](/docs/fr-CA/fr-ca/get-started/auth0-overview/dashboard).
</Callout>
...
Pour localiser les courriels, vous pouvez définir la variable `locale` à envoyer dans `user_metadata` lors de la création d’une invitation. Pour en savoir plus, consultez [Personnaliser les modèles de courriel : Modèles de courriels multilingues](/docs/fr-CA/fr-ca/customize/email/email-templates).
...
Vous devez [configurer une route de connexion par défaut](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login/configure-default-login-routes) vers laquelle les utilisateurs seront envoyés après avoir sélectionné le lien pour accepter l’invitation. Lors de la redirection, Auth0 inclura les paramètres suivants :
...
* [Personnaliser le traitement des courriels](/docs/fr-CA/fr-ca/customize/email/manage-email-flow)
* [Personnaliser les courriels des comptes bloqués](/docs/fr-CA/fr-ca/customize/email/customize-blocked-account-emails)