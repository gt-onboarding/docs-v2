...
* [Objet événement](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object) : fournit des informations contextuelles sur un seul utilisateur se connectant via Auth0.
* [Objet API](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object) : fournit des méthodes permettant de modifier le comportement du flux.
...
Auth0 fournit un système complet de stockage des métadonnées dans un [Profil utilisateur](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-profiles/normalized-user-profiles). Afin de définir `user_metadata` ou `app_metadata` sur le profil d’un utilisateur lors de sa connexion, utilisez les fonctions`api.user.setUserMetadata` ou `api.user.setAppMetadata`.
...
Une fois toutes les actions post-connexion exécutées, Actions mettra à jour le profil utilisateur en une seule opération. Cette opération est soumise à une [limite anti-attaques](/docs/fr-CA/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits) « Write Users » (Écriture des utilisateurs).

<div id="enforce-custom-mfa-policy">
  ### Appliquer la politique d’authentification multifacteur (MFA) personnalisée
</div>

Une Action post-connexion peut être utilisée pour exiger dynamiquement l’authentification multifacteur (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
...
Un fournisseur de MFA doit être configuré pour activer la MFA lors d’une connexion. Pour en savoir plus, veuillez consulter [Authentification multifacteur (MFA)](/docs/fr-CA/fr-ca/secure/multi-factor-authentication).
...
Pour en savoir plus à propos des Actions de redirection, veuillez consulter [Rediriger avec des actions](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions).
...
* [Rediriger avec des actions](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions)
