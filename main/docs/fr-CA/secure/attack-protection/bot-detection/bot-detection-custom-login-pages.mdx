...
4. Utilisez le modèle fourni pour commencer à personnaliser votre page de connexion.
   ...
   <Callout icon="file-lines" color="#0EA5E9" iconType="regular">
     Nous vous recommandons d’utiliser un logiciel de contrôle des versions pour gérer le code source de vos pages de connexion personnalisées. Pour en savoir plus, lisez [Contrôle des versions des pages de connexion classiques](/docs/fr-CA/fr-ca/customize/login-pages/classic-login/version-control).
   </Callout>
...
Lorsque vous appelez les méthodes `renderCaptcha` et `renderSignupCaptcha`, vous pouvez configurer le modèle pour chaque [fournisseur CAPTCHA pris en charge](/docs/fr-CA/fr-ca/secure/attack-protection/bot-detection/configure-captcha) à travers le paramètre `options`.
