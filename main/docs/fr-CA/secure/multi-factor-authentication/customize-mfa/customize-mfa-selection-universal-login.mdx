...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  ...
  * La personnalisation des flux MFA à l’aide d’actions n’est possible que si vous utilisez la [connexion universelle](/docs/fr-CA/fr-ca/authenticate/login/auth0-universal-login/universal-login-vs-classic-login/universal-experience).
</Callout>
...
Vous pouvez utiliser [Actions](/docs/fr-CA/fr-ca/customize/actions) pour personnaliser vos flux MFA. Plus particulièrement, vous pouvez modifier le déclencheur `post-connexion` du [Flux de connexion](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) à l’aide des méthodes Authentication API suivantes :
...
* [Déclencheurs d’actions : post-connexion - Objet d’événement](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-event-object)
* [Déclencheurs d’actions : post-connexion - Objet API](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object)
...
* Pour en apprendre davantage sur le processus de configuration, consultez [Activer l’authentification multifacteur (MFA)](/docs/fr-CA/fr-ca/secure/multi-factor-authentication/enable-mfa).
* Pour plus d’information à propos de la configuration de facteurs spécifiques, consultez [Facteurs d’authentification multifacteur (MFA)](/docs/fr-CA/fr-ca/secure/multi-factor-authentication/multi-factor-authentication-factors).
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  * Les Actions qui invoquent les commandes `challengeWith` et `challengeWithAny` remplacent et écrasent tous les défis activés via [api.multifactor.enable](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object). Elles ont également priorité sur les configurations MFA disponibles sous **Définir les politiques**.
  ...
  * Si vous souhaitez utiliser des évaluateurs de risques dans vos commandes, activez le bouton bascule **Évaluation de risque de l&#39;authentification multifacteur (MFA) adaptative** et utilisez [event.authentication.riskAssessment](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/post-login-api-object) dans votre code d’actions `post-login`.
</Callout>
...
Une fois qu’un utilisateur s’est inscrit ou qu’il a été créé dans votre locataire, vous pouvez créer des inscriptions avec le point de terminaison du <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip> [authentication-methods](/docs/fr-CA/fr-ca/api/management/v2/users/post-authentication-methods), ou vous pouvez gérer les inscriptions des utilisateurs directement par le biais de leur page de profil dans le Tableau de bord Auth0.
...
<Warning>
  ...
  * L’Action de redirection doit être la dernière Action exécutée dans votre flux.
  ...
  Pour plus d’informations sur les redirections, consultez [Rediriger avec des actions](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger/redirect-with-actions).
</Warning>
...
Dans cet exemple, les facteurs MFA sont activés au niveau de l’[Organisation](/docs/fr-CA/fr-ca/manage-users/organizations). Cet exemple utilise différentes catégories de métadonnées pour déterminer le défi approprié pour chaque utilisateur :
...
L’exemple suivant utilise une [Authentification multifacteur (MFA) adaptative](/docs/fr-CA/fr-ca/get-started/auth0-teams/tenant-management) pour déterminer si les utilisateurs devraient être mis à l’épreuve.
...
Vous pouvez surveiller vos flux MFA personnalisés par le biais des [journaux des locataires](/docs/fr-CA/fr-ca/deploy-monitor/logs).
