...
Cette fonctionnalité vous permet de personnaliser les flux d’inscription à la MFA. Si vous souhaitez personnaliser les flux de MFA pour les utilisateurs déjà inscrits, consultez [Personnaliser la sélection MFA pour la connexion universelle](/docs/fr-CA/fr-ca/secure/multi-factor-authentication/customize-mfa/customize-mfa-selection-universal-login).

<div id="how-it-works">
  ## Fonctionnement
</div>

Vous pouvez utiliser [Actions](/docs/fr-CA/fr-ca/customize/actions) pour personnaliser vos flux d’inscription à MFA. Plus particulièrement, vous pouvez modifier le déclencheur `post-connexion` du [Flux de connexion](/docs/fr-CA/fr-ca/customize/actions/explore-triggers/signup-and-login-triggers/login-trigger) à l’aide des méthodes d’Authentication API suivantes :
...
* Pour plus d’information à propos de la configuration de facteurs particuliers, consultez [Facteurs d’authentification multifacteur (MFA)](/docs/fr-CA/fr-ca/secure/multi-factor-authentication/multi-factor-authentication-factors).
...
Vous pouvez surveiller vos inscriptions personnalisées à la MFA grâce aux [journaux des locataires](/docs/fr-CA/fr-ca/deploy-monitor/logs).

Les journaux des locataires sont disponibles dans Auth0 Dashboard sous [Monitoring (Surveillance) &gt; Logs (Journaux)](https://manage.auth0.com/#/logs). Vous pouvez également récupérer les journaux à l’aide de [Management API](/docs/fr-CA/fr-ca/api/management/v2/introduction).
