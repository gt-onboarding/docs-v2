...
L’utilisation d’un `requestToken` ou d’un `ticket` dépend de la méthode d’authentification. Le ticket correspond à un [ticket d’inscription](/docs/fr-CA/fr-ca/secure/multi-factor-authentication/multi-factor-authentication-developer-resources/create-custom-enrollment-tickets)généré précédemment.
