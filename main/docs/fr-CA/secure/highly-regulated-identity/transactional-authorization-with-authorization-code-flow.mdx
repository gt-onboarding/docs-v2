...
Suivez les instructions de [Configurer les demandes d’autorisation enrichies (RAR)](/docs/fr-CA/fr-ca/get-started/apis/configure-rich-authorization-requests) pour l’API ou le serveur de ressources que vous souhaitez configurer :
...
* [Clé privée JWT](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authenticate-with-private-key-jwt)
* [mutual-TLS (mTLS) pour OAuth](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authenticate-with-mtls)
...
L’action post-connexion expose également `event.transaction.linking_id`, qui contient un identifiant universel unique (UUID) de la transaction. Plus tard, lorsque Auth0 demande à l’utilisateur d’approuver la transaction, `linking_id` fournit une référence pour [Liaison dynamique](/docs/fr-CA/fr-ca/secure/highly-regulated-identity#dynamic-linking). Vous pouvez également ajouter le `linking_id` au jeton d’accès en tant que [demande personnalisée](/docs/fr-CA/fr-ca/secure/tokens/json-web-tokens/create-custom-claims) pour associer les détails d’autorisation d’une transaction spécifique aux appels API de votre côté. Cela facilite la traçabilité, car Auth0 inclut le `linking_id` dans les journaux du locataire.
...
Envoyez une notification poussée à l’appareil mobile enrôlé d’un utilisateur pendant qu’Auth0 invite l’utilisateur à afficher l’écran d’attente de l’authentification multifacteur (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=multifactor-authentication" tip="Authentification multifacteur (MFA)
