...
Pour lire les demandes personnalisées sur les jetons d’accès et d’ID, vous devez utiliser des jetons Web JSON (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
Format standard de jeton d’ID (et souvent de jeton d’accès) utilisé pour représenter en toute sécurité des demandes entre deux parties." cta="Voir le glossaire">JWT</Tooltip>) et transmettre une <Tooltip data-tooltip-id="react-containers-DefinitionTooltip-1" tip="">audience</Tooltip> (`aud`) dans un flux de connexion OIDC. Pour en savoir plus, veuillez consulter la section [Jetons d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens).
...
Pour plus d’exemples de demandes personnalisées ajoutées à un jeton, veuillez consultez [Exemples de cas d’utilisation : Scopes and Claims (Permissions et demandes)](/docs/fr-CA/fr-ca/get-started/apis/scopes/sample-use-cases-scopes-and-claims).
