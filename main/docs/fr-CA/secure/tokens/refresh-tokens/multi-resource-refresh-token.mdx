...
Multi-Resource <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=refresh-token" tip="Jeton d’actualisation
Jeton utilisé pour obtenir un jeton d’accès renouvelé sans obliger les utilisateurs à se connecter à nouveau." cta="Voir le glossaire">Refresh Tokens</Tooltip> (MRRT) allows a single [refresh token](/docs/fr-CA/fr-ca/secure/tokens/refresh-tokens) to receive [access tokens](/docs/fr-CA/fr-ca/secure/tokens/access-tokens) for multiple [APIs](/docs/fr-CA/fr-ca/get-started/apis), each with their own scopes and permissions. MRRT builds on standard [OAuth 2.0](/docs/fr-CA/fr-ca/authenticate/protocols/oauth) behavior by allowing refresh tokens to maintain multiple authorization policies.

When an application exchanges a refresh token for an <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=access-token" tip="Jeton d’accès
...
* No need to go through a full <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=authorization-flow" tip="Flux d’autorisation
...
* Performance improvement and reduced load on the <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=authorization-server" tip="Serveur d’autorisations :
...
* Decrease risk of [rate limitation](/docs/fr-CA/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy) due to repeated full authorization code flows.
...
Learn how to [Configure and Implement Multi-resource refresh token](/docs/fr-CA/fr-ca/secure/tokens/refresh-tokens/multi-resource-refresh-token/configure-and-implement-multi-resource-refresh-token).
...
* The Auth0 <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
