...
Jusqu’à récemment, vous pouviez générer un jeton de Management APIv2 directement à partir de l’explorateur <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
La nouvelle méthode utilise le [flux des identifiants client](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow). Pour apprendre à utiliser le nouveau processus, consultez [Jetons d’accès à Management API](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens).
...
La nouvelle implémentation ne présente pas de tels risques. Une fois que vous avez effectué la configuration initiale, vous pouvez obtenir un jeton soit en visitant <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
Principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire">Auth0 Dashboard</Tooltip>, soit en effectuant une simple requête `POST` vers le [point de terminaison `/oauth/token` de notre Authentication API](/docs/fr-CA/fr-ca/api/authentication#client-credentials).
...
Pour obtenir un jeton, vous devez suivre uniquement le processus décrit dans [Jetons d’accès à Management API](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens).