...
Les clés de connexion sont utilisées pour signer les jetons d’ID, les jetons d’accès, les assertions <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=security-assertion-markup-language" tip="Security Assertion Markup Language (SAML)
...
Protocole de gestion des identités des utilisateurs à travers les domaines." cta="Voir le glossaire">WS-Fed</Tooltip> envoyées à votre application ou à votre API. La clé de connexion est une clé Web JSON (JWK) qui contient une clé publique bien connue, utilisée pour valider la signature d’un jeton Web JSON signé (<Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Ces liens sont renseignés en utilisant votre locataire actif pour vous fournir une information précise. Vous devez être connecté à [auth0.com/docs](/docs/fr-CA/fr-ca/articles) avec les identifiants de votre locataire pour accéder à ces liens.
...
Vous pouvez également récupérer ces informations pour des applications individuelles via <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
La rotation de votre clé de connexion sera soumise à une limite de débit inférieure à celle des autres points de terminaison de l’API. Pour en savoir plus, consultez [Limites anti-attaques de Management API](/docs/fr-CA/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits).
