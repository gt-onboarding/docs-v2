...
Dans Auth0, la meilleure méthode pour mettre en œuvre la multilocation est [Auth0 Organizations](/docs/fr-CA/fr-ca/manage-users/organizations/organizations-overview). Si nécessaire, d’autres solutions héritées peuvent être utilisées pour répondre à des cas d’utilisation distincts. Si vous proposez un produit ou un service [commerce interentreprises (B2B)](/docs/fr-CA/fr-ca/get-started/architecture-scenarios/business-to-business), la mise en place d’une multilocation pour vos utilisateurs professionnels peut s’avérer avantageuse pour votre cas d’utilisation.
...
Avec Auth0 Organizations, vous pouvez créer des groupes uniques d’utilisateurs et personnaliser leurs expériences avec un [contrôle d’accès basé sur les rôles (RBAC)](/docs/fr-CA/fr-ca/manage-users/access-control/rbac), [des pages de connexion et des modèles de courriel personnalisés](/docs/fr-CA/fr-ca/manage-users/organizations/create-first-organization#customize-prompts-and-email-templates), et plus encore.

Pour en savoir plus sur l’utilisation d’Auth0 Organizations pour mettre en œuvre la multilocation, consultez l’article [Architecture d’organizations multiples](/docs/fr-CA/fr-ca/get-started/architecture-scenarios/multiple-organization-architecture).

<div id="legacy-solutions">
  ## Solutions héritées
</div>

Si Auth0 Organizations ne répond pas aux exigences de votre cas d’utilisation, veuillez contacter notre équipe de [services professionnels](/docs/fr-CA/fr-ca/get-started/professional-services) pour trouver une solution qui assure votre succès.
...
* Vous avez des groupes d’utilisateurs provenant de connexions différentes. Par exemple, un locataire pourrait demander aux utilisateurs de fournir des identifiants nom d’utilisateur/mot de passe, tandis qu’un autre locataire pourrait demander aux utilisateurs de se connecter par l’intermédiaire d’un <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=idp" tip="Fournisseur d’identité (IdP)
...
Cette approche vous permet de partager l’accès à <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
