...
Auth0 prend en charge les restrictions d’accès pour les applications ou les API en fonction de certaines conditions. Dans certains scénarios, vous pourrez vouloir créer une règle qui renvoie une `UnauthorizedError` lorsque, par exemple, un utilisateur tente d’accéder à une application ou à une API à un moment inapproprié (comme décrit dans cet [exemple](/docs/fr-CA/fr-ca/authorization/sample-use-cases-rules-with-authorization#allow-access-only-on-weekdays-for-a-specific-application)), ou si l’utilisateur ne possède pas les bonnes demandes contenues dans ses `app_metadata`. Pour une application utilisant [OpenID Connect (OIDC)](/docs/fr-CA/fr-ca/authenticate/protocols/openid-connect-protocol), cela empêcherait l’attribution du [jeton d’ID](/docs/fr-CA/fr-ca/secure/tokens/id-tokens) utilisé pour autoriser l’accès. De même, pour une API, l’attribution de tout [jeton d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens) OAuth2 (utilisé lors de l’appel de l’API), pourrait être empêchée, comme décrit dans cet [exemple](/docs/fr-CA/fr-ca/authorization/sample-use-cases-rules-with-authorization#deny-access-to-anyone-calling-an-api).
...
Dans ce scénario, votre locataire Auth0 fournit un jeton comme indicateur d’accès autorisé à une application. Pour les applications utilisant [OpenID Connect (OIDC)](/docs/fr-CA/fr-ca/authenticate/protocols/openid-connect-protocol), le protocole standard de l’industrie généralement le plus utilisé pour les applications destinées aux clients serait un jeton d’ID exprimé sous forme de [JWT](/docs/fr-CA/fr-ca/secure/tokens/json-web-tokens).

<div id="id-token-claims">
  ### Demandes relatives aux jetons d’ID
</div>

En utilisant l’extensibilité des règles, Auth0 vous permet de facilement [ajouter des demandes personnalisées à un jeton d’ID](/docs/fr-CA/fr-ca/scopes/sample-use-cases-scopes-and-claims#add-custom-claims-to-a-token) en fonction, par exemple, du contenu des [métadonnées](/docs/fr-CA/fr-ca/manage-users/user-accounts/metadata) d’un utilisateur. Votre application peut ensuite vérifier le jeton d’ID pour les demandes nécessaires et soit autoriser, soit empêcher l’accès à certaines fonctionnalités selon les besoins. Notez que, bien que le processus d’ajout de revendications personnalisées via les règles soit simplifié, le moteur des règles est flexible et vous permet d’élaborer du code personnalisé qui peut avoir des effets négatifs. Il est donc important de suivre nos conseils sur les [meilleures pratiques en matière de règles](/docs/fr-CA/fr-ca/rules-best-practices) chaque fois que vous utilisez cette fonctionnalité d’extensibilité.
...
Alternativement, vous pouvez avoir des données relatives à un utilisateur pour lesquelles des [fonctionnalités supplémentaires sont fournies](/docs/fr-CA/fr-ca/scopes/api-scopes#example-an-api-called-by-a-back-end-service) et pour lesquelles un consentement d’utilisateur explicite ne peut pas être obtenu (c’est-à-dire qu’il n’y a pas d’utilisateur authentifié qui puisse le fournir). Dans ce scénario, une [liste d’applications pour lesquelles l’octroi des identifiants client est activé peut être](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow) définie.

<div id="access-token-claims">
  ### Demandes relatives aux jetons d’accès
</div>

Comme c’est le cas avec les jetons d’ID, vous pouvez [ajouter des demandes personnalisées aux jetons d’accès](/docs/fr-CA/fr-ca/scopes/sample-use-cases-scopes-and-claims#add-custom-claims-to-a-token) en utilisant l’extensibilité des règles Auth0. Une fois ajoutée, votre API peut ensuite vérifier un jeton d’accès pour les demandes nécessaires et soit autoriser, soit empêcher l’accès à certaines fonctionnalités selon les besoins.
...
Avec <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=fine-grained-auth" tip="Autorisation à granularité fine (FGA)
...
Auth0 permet le contrôle d’accès basé sur les rôles([RBAC](/docs/fr-CA/fr-ca/manage-users/access-control/rbac)). RBAC fait référence à l’attribution d’autorisation aux utilisateurs en fonction de leur rôle au sein d’une organisation, et offre un contrôle d’accès plus simple en proposant une approche plus gérable, moins sujette à erreur.

<div id="machine-to-machine-m2m-authorization">
  ## Autorisation machine-machine (M2M)
</div>

Il existe de nombreux scénarios qui nécessitent qu’une application sans session utilisateur interactive obtienne un jeton d’accès afin d’appeler une API. Dans de tels scénarios, vous devez authentifier le client au lieu de l’utilisateur, et <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=oath2" tip="OAuth 2.0
Cadre d’applications d’autorisation qui définit les protocoles d’autorisation et les flux de production." cta="Voir le glossaire">OAuth</Tooltip> 2 fournit le type d’octroi des [identifiants client](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow) pour faciliter le processus. Quelques exemples courants où cela est nécessaire :
