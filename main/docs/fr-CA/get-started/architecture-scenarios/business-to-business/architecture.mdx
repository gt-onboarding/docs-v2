...
Pour les applications orientées client, nous constatons généralement que [OpenID Connect (OIDC)](/docs/fr-CA/fr-ca/authenticate/protocols/openid-connect-protocol) est le protocole le plus fréquemment utilisé. L’OIDC utilise des flux de travail Web avec des URL de navigateur qui sont présentés à l’utilisateur. Dans le cadre de la prise en charge d’Auth0 OIDC, les URL orientées client sont de marque Auth0, mais nous recommandons d’utiliser la fonctionnalité [domaine personnalisé](#custom-domains) d’Auth0 afin de garantir une identité d’entreprise cohérente et de répondre aux problèmes potentiels de confiance des utilisateurs avant qu’ils ne se posent.
