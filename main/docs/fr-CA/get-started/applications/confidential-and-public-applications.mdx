...
Pour en savoir plus, consulter [Vérification du caractère confidentiel ou public d’une application](/docs/fr-CA/fr-ca/get-started/applications/confidential-and-public-applications/view-application-type).
...
Les applications confidentielles utilisent un serveur dorsal de confiance et peuvent utiliser des types d’autorisation exigeant de spécifier leur ID Client et Secret client (ou autres identifiants enregistrés alternatifs) pour s’authentifier en appelant le point de terminaison [Obtenir un jeton](https://auth0.com/docs/api/authentication#get-token) de l’Authentication API Auth0. Les applications confidentielles peuvent utiliser les méthodes d’authentification Publication du secret client <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=json-web-token" tip="Jeton Web JSON (JWT)
Format standard de jeton d’ID (et souvent de jeton d’accès) utilisé pour représenter en toute sécurité des demandes entre deux parties." cta="Voir le glossaire">JWT</Tooltip>, Secret client de base ou [Clé privée JWT](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authenticate-with-private-key-jwt).

Les applications suivantes sont considérées comme des applications confidentielles :

* Une application web avec un système dorsal sécurisé qui utilise le[Flux de code d’autorisation](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow), [Flux de mot de passe du propriétaire de ressource](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/resource-owner-password-flow) ou Flux de mot de passe du propriétaire de ressource avec soutien de partition
* Une application de communication entre machines (M-M) qui utilise le [Flux des identifiants client](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/client-credentials-flow)
...
* Une application de bureau ou mobile native qui utilise [Flux de code d’autorisation avec PKCE](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authorization-code-flow-with-pkce)
* Une application web côté client basée sur JavaScript (telle qu’une application à page unique) qui utilise le [flux d’autorisation implicite](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/implicit-flow-with-form-post)

<div id="id-tokens">
  ### Jetons d’ID
</div>

Les applications publiques ne pouvant contenir de secrets, les [jetons d’ID](/docs/fr-CA/fr-ca/secure/tokens/id-tokens) qui leur sont délivrés doivent être :
...
* [Vérification du caractère confidentiel ou public d’une application](/docs/fr-CA/fr-ca/get-started/applications/confidential-and-public-applications/view-application-type)
