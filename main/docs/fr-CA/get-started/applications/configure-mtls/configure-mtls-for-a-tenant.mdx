...
<Callout icon="file-lines" color="#0EA5E9" iconType="regular">
  Les exemples suivants spécifient **$management&#95;access&#95;token**, ou un [jeton d’accès de Management API](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/management-api-access-tokens). Il doit être remplacé par un jeton d’accès contenant au moins les permissions suivantes :
  ...
  * `update:tenant_settings`
  ...
  Pour en savoir plus sur la récupération d’un jeton d’accès avec les permissions requises, lisez [Obtenir des jetons d’accès](/docs/fr-CA/fr-ca/secure/tokens/access-tokens/get-access-tokens).
</Callout>

Pour commencer, vous devez configurer et vérifier un [domaine personnalisé](/docs/fr-CA/fr-ca/customize/custom-domains).

<div id="create-a-custom-domain">
  ### Création du domaine personnalisé
</div>

Au niveau du locataire, vous devez configurer un [domaine personnalisé](/docs/fr-CA/fr-ca/customize/custom-domains) pour accepter des en-têtes mTLS avec [Management API](https://auth0.com/docs/api/management/v2). Étant donné que l’[avantage client](/docs/fr-CA/fr-ca/get-started/applications/configure-mtls/set-up-the-customer-edge) est responsable pour valider les certificats du client, réglez le `type` comme `self_managed_certs` dans la requête POST :
...
Les alias de point de terminaison mTLS indiquent que les clients devraient envoyer le trafic mTLS aux points de terminaison indiqués dans la propriété `mtls_endpoint_aliases` du document de découverte OIDC. Les clients enverront le trafic non-mTLS aux points de terminaison normaux. Pour obtenir plus d’information sur la propriété `mtls_endpoint_aliases`, consultez [Appeler le serveur de ressources](/docs/fr-CA/fr-ca/get-started/authentication-and-authorization-flow/authenticate-with-mtls#call-the-resource-server).

Vous pouvez activer les alias de points de terminaison mTLS avec <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
Principal produit d’Auth0 pour configurer vos services." cta="Voir le glossaire">Auth0 Dashboard</Tooltip> (Tableau de bord Auth0) et <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    1. Naviguez jusqu’à [Auth0 Dashboard](/docs/fr-CA/fr-ca/%24manage_url/#) et sélectionnez **Settings (Paramètres)** dans le menu latéral.
...
* [Configurer le réseau de périphérie client](/docs/fr-CA/fr-ca/get-started/applications/configure-mtls/set-up-the-customer-edge)
* [Configurer l’authentification mTLS pour un client](/docs/fr-CA/fr-ca/get-started/applications/configure-mtls/configure-mtls-for-a-client)