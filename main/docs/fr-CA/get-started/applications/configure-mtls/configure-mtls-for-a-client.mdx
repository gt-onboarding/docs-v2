...
<Tabs>
  <Tab title="Auth0 Dashboard">
    ...
    2. Sélectionnez l’application que vous souhaitez utiliser avec mTLS ou [créez une nouvelle application](/docs/fr-CA/fr-ca/get-started/auth0-overview/create-applications).
    3. Sélectionnez l’onglet **Credentials (Identifiants)**
    4. Choisissez la **méthode d’authentification** requise, qui peut être :

       * mTLS avec un [certificat autosigné](/docs/fr-CA/fr-ca/customize/custom-domains/self-managed-certificates), ou
  ...
  <Tab title="Management API">
    ...
    * `$credential_certificate` : le contenu de `$certificate_pem`généré à l’étape précédente
    ...
    La méthode de génération d’un certificat client signé par une autorité de certification dépend fortement de l’infrastructure de clés publiques et dépasse le cadre du présent document. Nous recommandons de générer une [paire de clés RSA d’au minimum 2048 bits](/docs/fr-CA/fr-ca/secure/application-credentials/generate-rsa-key-pair).
    ...
    * `$credential_certificate` : le contenu de `$certificate_pem` généré par l’autorité de certification
    ...
    Une fois que vous avez [généré une paire de clés](/docs/fr-CA/fr-ca/secure/application-credentials/generate-rsa-key-pair) exclusivement pour mTLS, créez la ressource d’authentification. Effectuez la requête POST suivante vers le point de terminaison `/clients` :
...
* [Configurer l’authentification mTLS pour un locataire](/docs/fr-CA/fr-ca/get-started/applications/configure-mtls/configure-mtls-for-a-tenant)