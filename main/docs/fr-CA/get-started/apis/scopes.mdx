...
* À partir d’une application, pour vérifier l’identité d’un utilisateur et obtenir des informations de profil de base sur l’utilisateur, telles que son courriel ou sa photo. Dans ce scénario, les permissions à votre disposition incluent celles implémentées par le protocole <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=openid" tip="OpenID
  Norme ouverte d’authentification qui permet aux applications de vérifier l’identité des utilisateurs sans avoir à collecter leurs informations de connexion." cta="Voir le glossaire">OpenID</Tooltip> Connect (OIDC). Pour en savoir plus, veuillez consulter [Permissions OpenID Connect](/docs/fr-CA/fr-ca/get-started/apis/scopes/openid-connect-scopes).
* Dans une [API](/docs/fr-CA/fr-ca/api), pour mettre en œuvre le contrôle d’accès. Dans ce cas, vous devez définir des permissions personnalisées pour votre API, puis identifier ces permissions afin que les applications appelantes puissent les utiliser. Pour en savoir plus, veuillez consulter [Permissions des API](/docs/fr-CA/fr-ca/get-started/apis/scopes/api-scopes).
* Depuis une application, pour appeler une API qui a implémenté ses propres permissions personnalisées. Dans ce cas, vous devez savoir quelles permissions personnalisées sont définies pour l’API que vous appelez. Pour voir des exemples d’appel d’une API personnalisée à partir d’une application, veuillez consulter [Exemples de cas d’utilisation : Permissions et demandes](/docs/fr-CA/fr-ca/get-started/apis/scopes/sample-use-cases-scopes-and-claims)
