...
En règle générale, vous [déboguez une règle pendant l’exécution](/docs/fr-CA/fr-ca/customize/rules/debug-rules) via la journalisation de la console en utilisant la fonction `console.log`. Pour en savoir plus, lisez [console.log() dans MDN Web Docs](https://developer.mozilla.org/en-US/docs/Web/API/Console/log). Le débogage interactif d’une règle n’est pas disponible au sein de la plateforme Auth0 (bien que l’on puisse utiliser la technique d’automatisation des tests décrite ci-dessous en conjonction avec une fonction externe de débogage interactif des sources; pour en savoir plus, lisez les [Meilleures pratiques en matière de test des règles](/docs/fr-CA/fr-ca/rules-best-practices/rules-testing-best-practices)).
...
* [Meilleures pratiques des performances](/docs/fr-CA/fr-ca/troubleshoot/performance-best-practices)