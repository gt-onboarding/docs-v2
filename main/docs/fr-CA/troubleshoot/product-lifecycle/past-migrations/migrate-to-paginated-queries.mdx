...
Après le **26 janvier 2021** (ou Private Cloud version 2202), les requêtes adressées aux points de terminaison de <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
...
Vous devrez peut-être mettre à jour les versions précédentes des [extensions Auth0](/docs/fr-CA/fr-ca/customize/extensions) et des extensions personnalisées vers les dernières versions pour vous assurer qu’elles effectuent uniquement des requêtes paginées.
...
Si vous utilisez l’outil `auth0-deploy-cli` , vous devez mettre à jour vers la version **5.3.0 ou ultérieure**. Pour en savoir plus, consultez [Déployer l’outil Deploy CLI](/docs/fr-CA/fr-ca/deploy-monitor/deploy-cli-tool).