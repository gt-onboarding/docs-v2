...
Les métadonnées client d’une application peuvent être définies manuellement via <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=auth0-dashboard" tip="Auth0 Dashboard
...
Essayez d’éviter les appels à <Tooltip href="/docs/fr-CA/fr-ca/glossary?term=management-api" tip="Management API
Un produit permettant aux clients d’effectuer des tâches administratives." cta="Voir le glossaire">Management API</Tooltip> Auth0. Management API Auth0 est limitée en termes de taux, un facteur à prendre en compte même lorsque l’on utilise l’objet `auth0` (veillez donc à l’utiliser avec parcimonie). Pour en savoir plus, consultez [Limites anti-attaques des points de terminaison de Management API.](/docs/fr-CA/fr-ca/troubleshoot/customer-support/operational-policies/rate-limit-policy/management-api-endpoint-rate-limits)
...
* [Meilleures pratiques liées aux règles](/docs/fr-CA/fr-ca/rules-best-practices)
