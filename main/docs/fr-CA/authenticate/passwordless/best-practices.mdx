...
* [Connexion universelle sans mot de passe](/docs/fr-CA/fr-ca/authenticate/passwordless/passwordless-with-universal-login)
* [Passwordless Authentication with Universal Login (Authentification sans mot de passe avec la connexion universelle)](/docs/fr-CA/fr-ca/authenticate/passwordless/implement-login/universal-login)
* [Passwordless Authentication with Embedded Login (Authentification sans mot de passe avec connexion intégrée)](/docs/fr-CA/fr-ca/authenticate/passwordless/implement-login/embedded-login)
...
Pour ces raisons, nous vous recommandons, si vous utilisez l’authentification sans mot de passe, de mettre également en œuvre [Authentification multifacteur (MFA)](/docs/fr-CA/fr-ca/secure/multi-factor-authentication) avec un facteur différent lorsque l’utilisateur effectue une opération sensible pour la sécurité.
...
* Si un utilisateur de connexion sans mot de passe a été bloqué [administrativement](/docs/fr-CA/fr-ca/manage-users/user-accounts/block-and-unblock-users), Auth0 n’enverra aucun code OTP par SMS ou courriel jusqu’à ce que l’utilisateur ait été débloqué. Ce comportement protège vos fournisseurs de SMS et de courriel en leur évitant de recevoir des demandes inutiles.
...
Si vous activez **Disable Signs Ups (Désactiver les inscriptions)**, [Brute Force Protection (Protection contre les attaques exhaustives)](/docs/fr-CA/fr-ca/secure/attack-protection/brute-force-protection) peut atténuer la menace de certaines de ces attaques, mais elle ne protège pas complètement votre application.

<div id="link-accounts">
  ## Liaison de comptes
</div>

Les utilisateurs peuvent vouloir s’authentifier à l’aide de différents facteurs sans mot de passe au cours de leur vie. Par exemple, ils pourraient d’abord s’inscrire par SMS, puis commencer à s’authentifier par courriel. Vous pouvez y parvenir en leur permettant de lier leurs différents profils en utilisant [account linking (liaison de compte)](/docs/fr-CA/fr-ca/manage-users/user-accounts/user-account-linking/link-user-accounts).
