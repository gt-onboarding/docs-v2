...
export const SignUpForm = () => {
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      unsubscribe = window.autorun(() => {
        const authenticated =
          window.rootStore?.sessionStore?.isAuthenticated || false
   ...
   * - Structure prête pour la production, Tailwind seulement
   ...
   *   3) Intégrer avec une application existante : sélectionner l'application + enregistrer les URL -> notification
   */
  function LoggedInForm({ sampleApp }) {
    ...
    function loadApps() {
      ...
      // initialiser avec les 3 affichées dans les captures d'écran
    ...
    function Card({ className = "", children }) {
      return (
        <div
          className={`rounded-2xl shadow-sm ring-1 ring-zinc-200 dark:ring-zinc-800 ${className}`}
        >
          {children}
        </div>
      )
    ...
    function Button({
      ...
      const base =
        "inline-flex items-center justify-center gap-2 h-10 px-4 rounded-xl font-medium transition"
        ...
        <label className='block space-y-1 max-w-[300px]'>
          ...
          <div className='relative'>
            <select
              ...
              onChange={(e) => onChange(e.target.value)}
            >
              <optgroup label='Applications génériques'>
                {options.map((o) => (
      ...
      return (
        <div>
          {route === "menu" && (
            <Menu
              onCreate={() => nav("create")}
              onIntegrate={() => nav("integrate")}
            />
          )}

          {route === "create" && (
            <CreateForm onCancel={() => nav("menu")} onSave={onCreate} />
          )}
    ...
    return (
      <div className='flex flex-col gap-2 items-center h-full'>
        ...
        >
          ...
          à votre compte existant pour l'intégrer directement à votre propre locataire.
        </span>
        <button
