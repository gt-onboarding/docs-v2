...
export const LibrariesCards = ({ cards }) => {
  ...
  const Card = ({ item }) => {
    const github = getLink(item, "github")
    const sample = getLink(item, "サンプルアプリ")
    const クイックスタート = getLink(item, "クイックスタート")
    const docs = getLink(item, "始める")
    ...
    const tertiary = クイックスタート || docs
    const tertiaryLabel = クイックスタート ? "クイックスタート" : docs ? "始める" : ""
